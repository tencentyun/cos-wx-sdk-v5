...
var group = (function (name, fn) {
  ...
  var startTest = function () {
    serial(
      groupQueue,
      function (groupItem, groupIndex, nextGroup) {
        // 按顺序执行每个 group
        var testQueue = [];
        ...
        setTimeout(function () {
          serial(
            testQueue,
            function (testItem, testIndex, nextTest) {
              var assert = {
                ok: function (result, msg) {
                  result ? successCount++ : errorCount++;
                  var message = [
                    '  ' + (groupIndex + 1) + '.' + (testIndex + 1),
    ...
    cos.putObject(
      ...
      function (err, data) {
        ...
        cos.getObject(
          ...
          function (err, data) {
            var isSame = unescape(encodeURIComponent(data.Body))
              .split(0)
              .every((v, i) => v.charCodeAt(0) === arr[i]);
